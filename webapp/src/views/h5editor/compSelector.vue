<template>
	<div class="cs-wrap">
    <div class="clearfix">
      <span class="panel" :class="{ active: viewState === 0 }" @click="function () { viewState = 0 }">组件</span>
      <span class="panel" :class="{ active: viewState === 1 }" @click="function () { viewState = 1 }">页面</span>
      <span class="panel" :class="{ active: viewState === 2 }" @click="function () { viewState = 2 }">图层</span>
    </div>
		<div class="list custom-scroller" style="z-index: 1;">
			<div v-for="ele in elementTypeList">
				<div @click="onAddElement(ele.type)">{{ele.name}}</div>
			</div>	
		</div>
		<ul class="list custom-scroller" style="z-index: 2;" v-show="viewState == 1">
      <li v-for="page in pages">
		...
		<div class="list custom-scroller" style="z-index: 3;" v-show="viewState == 2">
      <ul>
...
export default {
	...
	computed: {
		...
		elementTypeList () {
			return this.$store.state.editor.elementTypeList		
		}	
	},
	...
	methods: {
		onAddElement (type) {
			switch (type) {
				case appConst.ELE_TITLE:
					this.addTitleElement()
					break
				case appConst.ELE_INPUT:
					this.addInputElement()
					break
				case appConst.ELE_TEXTAREA:
					this.addTextareaElement()
					break
				case appConst.ELE_BUTTON:
      ...
      this.$store.dispatch('addTitleElement', param)
		},
      ...
      this.$store.dispatch('addInputElement', param)
		},
      ...
      this.$store.dispatch('addTextareaElement', param)
		},
      ...
      this.$store.dispatch('addButtonElement', param)
		},
